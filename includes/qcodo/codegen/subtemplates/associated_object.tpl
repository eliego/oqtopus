		...
		*/ 
		public function Get<%= $objReverseReference->ObjectDescription %>Array($strOrderBy = null, $strLimit = null, $objExpansionMap = null) {
			if (<%= $objCodeGen->ImplodeObjectArray(' || ', '(is_null($this->', '))', 'VariableName', $objTable->PrimaryKeyColumnArray) %>)
				return array();

			return <%= $objReverseReference->VariableType %>::LoadArrayBy<%= $objReverseReferenceColumn->PropertyName %>(<%= $objCodeGen->ImplodeObjectArray(', ', '$this->', '', 'VariableName', $objTable->PrimaryKeyColumnArray) %>, $strOrderBy, $strLimit, $objExpansionMap);
		}
